  ...
  ICreditDelegationToken
{
  ...
  function transferDebt(address delegator, address fromUser, address toUser, uint256 amount) public {
    ...
    require(toUser != address(0), "DebtToken: receiver should not be zero address");
    require(amount > 0, "DebtToken: transfer amount must be greater than zero");
    require(_borrowAllowances[delegator][fromUser]>amount, "DebtToken: amount should not bigger than allowence");
    uint256 newFromUser= _borrowAllowances[delegator][fromUser] - amount;
    _borrowAllowances[delegator][fromUser]= newFromUser;
    emit BorrowAllowanceDelegated(delegator, fromUser, _underlyingAsset, newFromUser);
    uint256 newToUser= _borrowAllowances[delegator][toUser] + amount;
    if(_borrowAllowances[delegator][toUser]==0)
      _borrowAllowances[delegator][toUser]= newToUser;
    else _borrowAllowances[delegator][toUser] += newToUser;
    emit BorrowAllowanceDelegated(delegator, toUser, _underlyingAsset, newToUser);
  }
}
